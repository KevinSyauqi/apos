...
class _PrediksiPenjualanState extends State<PrediksiPenjualan>
  ...
  void initState() {
    super.initState();
    PredictionBloc _predictionBloc = BlocProvider.of<PredictionBloc>(context);
    _predictionBloc.add(GetPredictionOutletMenu());
  }
            ...
            body: BlocBuilder<PredictionBloc, PredictionState>(
              builder: (context, state) {
                ...
                if (state is PredictionEmpty) {
                  return Center(child: Text("Data penjualan belum lengkap"));
                }
...
Widget buildListPerhitunganPenjualan(List<Prediction> listPrediction) {
  return ListView.builder(
      itemCount: listPrediction.length,
      itemBuilder: (_, index) {
        Prediction prediction = listPrediction[index];
        return Container(
            decoration: BoxDecoration(
              border: Border(
                  bottom: BorderSide(
...
//                                      fontSize: 14.0,
//                                      fontFamily: 'CircularStd-Book')),
                              Text(prediction.wma.round().toString() + "pcs",
                                  style: TextStyle(
                                      color: Colors.black,
                                      fontSize: 16.0,
                                      fontFamily: 'CircularStd-Bold')),
